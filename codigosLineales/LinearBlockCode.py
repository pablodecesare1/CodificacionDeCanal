...
class LinearBlockCode:
    ...
    def dmin(self, Verbose = False):
        ...
        M = self.M()
        if Verbose:
            print("Empezamos con dmin = n")
            print("dmin = ", dmin)
            print("Iteracion a traves de la tabla de palabras de codigo y comparacion de los pesos de cada vector")
        for m in M:
            c = self.c(m)
            if w(c) != 0 and w(c) < dmin:
                if Verbose: print("El peso de ", c , " es ",w(c), " < ",dmin, " actualizamos dmin, dmin = ", w(c))
                dmin = w(c)
            else:
                if Verbose:print("Vector ", c, " tiene un peso de ", w(c))
    ...
    def errorDetectionCapability(self):
        """Error Detection Capability of the Block Code.
        """
        return self.dmin() - 1
    ...
    def Ai(self, i):
        """Weight distribution = number of codewords of weight i 
        Args:
            i: weight
        """
        C = self.C()
    ...
    def printParityCheckEquations(self):
        """Print the parity check equations for the linear block 
        """
        G = self.G()
